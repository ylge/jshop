...
public class ZtcSellerAction {
	...
	@Autowired
	private IZTCGoldLogService  zGoldLogService;
	...
	@SecurityMapping(title = "直通车购买日志", value = "/seller/ztc_gold_log_list.htm*", rtype = "seller", rname = "竞价直通车", rcode = "ztc_seller", rgroup = "促销推广")
	@RequestMapping("/seller/ztc_gold_log_list.htm")
	public ModelAndView ztc_gold_log_list(HttpServletRequest request,
		...
		if (!this.configService.getSysConfig().isZtc_status()) {
			...
			mv = new JModelAndView(
					"user/default/sellercenter/ztc_gold_log_list.html",
					configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 0, request, response);
			ZTCGoldLogQueryObject qo=new ZTCGoldLogQueryObject(currentPage, mv, "", "");
			qo.addQuery("obj.user_name",new SysMap("user_name",SecurityUserHolder.getCurrentUser().getUserName()),"=");
			qo.setOrderBy("addTime");
			qo.setOrderType("desc");
			IPageList pList=this.zGoldLogService.list(qo);
			CommUtil.saveIPageList2ModelAndView("", "", "", pList, mv);
		}
		return mv;
	}
