					...
					for (String city : city_list) {
						...
						if (city.equals(city_name)
							...
							if (trans_type == 1) {// 按照重量计算运费用
								double total_weight=CommUtil.null2Double(goods_weight)*count;
								if (CommUtil.null2Double(total_weight) > CommUtil
							...
							if (trans_type == 2) {// 按照体积计算运费用
								double total_volume=CommUtil.null2Double(goods_volume)*count;
								if (CommUtil.null2Double(total_volume) > CommUtil.null2Double(trans_weight)) {
									fee = trans_fee;
									float other_price = 0;
									if (trans_add_weight > 0) {
										other_price = trans_add_fee
												* Math.round(Math.ceil(CommUtil
														.subtract(total_volume,
																trans_weight)
														/ trans_add_weight));
									}
									fee = fee + other_price;
					...
					for (Map map : list) {
						...
						for (String city : city_list) {
							if (city.equals("全国")) {
								...
								if (trans_type == 1) {// 按照重量计算运费用
									double total_weight=CommUtil.null2Double(goods_weight)*count;
									if (CommUtil.null2Float(total_weight) > CommUtil
											.null2Double(trans_weight)) {
										fee = trans_fee;
										float other_price = 0;
										if (trans_add_weight > 0) {
								...
								if (trans_type == 2) {// 按照体积计算运费用
									double total_volume=CommUtil.null2Double(goods_volume)*count;
									if (CommUtil.null2Float(total_volume) > CommUtil
											.null2Double(trans_weight)) {
										fee = trans_fee;
										float other_price = 0;
										if (trans_add_weight > 0) {
