			...
			if (fileName.equals("")) {
				if (map2.get("fileName") != "") {
					...
					if(gc_ad!=null&&!"".equals(gc_ad)){
						Map map_t=Json.fromJson(Map.class, gc_ad);
						map_t.put("acc_url", gc_adv_url);
						goodsClass.setGc_advert(Json.toJson(map_t,
								JsonFormat.compact()));
					}
				}
			} else {
		...
		for (String id : ids) {
			if (!id.equals("")) {
				...
				List<Goods> goods_list = this.goodsService
						.query("select obj.id from Goods obj where obj.gc.id in (:ids) order by addTime asc",
								params, -1, -1);
				if (goods_list.size() == 0) {// 2016版开始，只有没有关联任何商品的分类信息才可以删除
					List<GoodsClass> gcs = this.goodsClassService
